...
class LocalFilemanager extends BaseFilemanager
{
	...
	protected $dynamic_fileroot = ''; // second part of the path : '/Filemanager/assets/' ( doc_root - $_SERVER['DOCUMENT_ROOT'])
	protected $path_to_files = "/uploads"; // path to FM userfiles folder - automatically computed by the PHP class, something like '/var/www/Filemanager/userfiles'
	protected $connector_script_url = 'mediamanager/ajax';
	...
	protected function loadLanguageFile()
    {
		...
		if(file_exists($this->fm_path . '/assets/filemanager/scripts/languages/'.$lang.'.json')) {
			$stream =file_get_contents($this->fm_path . '/assets/filemanager/scripts/languages/'.$lang.'.json');
			$this->language = json_decode($stream, true);
		} else {
			$l = substr($lang,0,2); // we try with 2 chars language file
			if(file_exists($this->fm_path. '/assets/filemanager/scripts/languages/'.$l.'.json')) {
				$stream = file_get_contents($this->fm_path . '/assets/filemanager/scripts/languages/'.$l.'.json');
				$this->language = json_decode($stream, true);
			} else {
				// we include default language file
				$stream = file_get_contents($this->fm_path . '/assets/filemanager/scripts/languages/'.$this->config['options']['culture'].'.json');
				$this->language = json_decode($stream, true);
			}
		}
	}
    ...
    {
		if ($handle = opendir($this->fm_path . '/assets/filemanager/scripts/languages/')) {
			while (false !== ($file = readdir($handle))) {
				if ($file != "." && $file != "..") {
